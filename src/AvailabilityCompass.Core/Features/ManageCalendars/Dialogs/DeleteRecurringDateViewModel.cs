...
public class DeleteRecurringDateViewModel : BaseDialogCrudViewModel<RecurringDateViewModel>
{
    private readonly IMediator _mediator;

    public DeleteRecurringDateViewModel(IMediator mediator, INavigationService<IDialogViewModel> dialogNavigationService) : base(dialogNavigationService)
    {
        _mediator = mediator;
    }
    ...
    protected override async Task<IProcessResult> ProcessDataAsync(CancellationToken ct)
    {
        ...
        return await _mediator.Send(new DeleteRecurringDateFromDbRequest(SelectedItem.CalendarId, SelectedItem.RecurringDateId),
            ct);
    }
}